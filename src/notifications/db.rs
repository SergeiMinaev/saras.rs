	...
	pub async fn page(&self, recipient_id: i32, offset: i32, size: i32) -> Vec<Notification> {
		let q = "select row_to_json(data) from (
			select
				...
				to_char(last_event_at, 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"') as last_event_at,
				case when read_at is not null then to_char(read_at, 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"') else null end as read_at
			from notifications
			where recipient_id = $1::INT
			order by (read_at is null) desc, last_event_at desc
